...
try {
    ...
    // Load config for database names
    ...
    foreach ($commonTables as $tableName) {
        ...
        if (!empty($primaryKeys)) {
            ...
            if (!isset($comparison['error'])) {
                $tableReport['records'] = [
                    ...
                    'different' => $comparison['differentData'] ?? 0,
        ...
        // var_dump($tableReport['records']);die;
        ...
        $hasDifferences =
            ...
            ($tableReport['records']['different'] ?? 0) > 0;
    ...
    usort($report['tables'], function ($a, $b) {
        $diffA = $a['records']['missing_in_a'] + $a['records']['missing_in_b'] + ($a['records']['different'] ?? 0);
        $diffB = $b['records']['missing_in_a'] + $b['records']['missing_in_b'] + ($a['records']['different'] ?? 0);
        return $diffB - $diffA;
    });
